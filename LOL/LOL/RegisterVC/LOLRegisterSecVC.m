...
#import "LOLRegisterThrVC.h"
#import <SMS_SDK/SMSSDK.h>

@implementation LOLRegisterSecVC
-(void)viewDidLoad
{
    [super viewDidLoad];
     self.title = @"注册2/3";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        UIButton *registerBtn = [UIButton buttonWithFrame:CGRectMake(15, 125, SCREEN_WIDTH-30,40) title:@"下一步，设置密码" backgroundImage:IMAGENAMED(@"pay_btn_orange_on")];
...
-(void)pushRegisterVC
{
      // 没有被注册可以注册
            [SMSSDK commitVerificationCode:self.phoneTf.text phoneNumber:self.phoneNum zone:@"86" result:^(NSError *error) {
                if (!error) {
                    NSLog(@"验证成功");
                    LOLRegisterThrVC *registerThree = [[LOLRegisterThrVC alloc] init];
                    registerThree.loadType = YWBasePresentType;
                    [self presentViewController:registerThree animated:YES completion:nil];
...
-(void)dealTimer{
    ...
    //发送通知
    [[NSNotificationCenter defaultCenter]postNotificationName:TIMECOUNT object:self userInfo:@{@"count":tiemNum}];
